...
%define main_release                 1_%{?openssl_version}%{?dist}

%define module_xslt_version          %{main_version}
%define module_xslt_release          %{main_release}
%define module_geoip_version         %{main_version}
%define module_geoip_release         %{main_release}
%define module_image_filter_version  %{main_version}
%define module_image_filter_release  %{main_release}
%define module_perl_version          %{main_version}
%define module_perl_release          %{main_release}
%define module_njs_shaid             1c50334fbea6
%define module_njs_version           %{main_version}.0.0.20160414.%{module_njs_shaid}
%define module_njs_release           %{main_release}

%define bdir %{_builddir}/nginx-%{main_version}
...
%define COMMON_CONFIGURE_ARGS $(echo "\
        ...
        --with-openssl="$RPM_BUILD_DIR/openssl-%{openssl_version}" \
...
Group: %{_group}
Requires: %{name} = %{?epoch:%{epoch}:}%{main_version}-%{main_release}
...
%prep
%setup -q -b 20 -n nginx-%{main_version}
tar xvzf %SOURCE13
cp %{SOURCE2} .
